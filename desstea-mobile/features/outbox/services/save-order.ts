...
export async function saveOrderLocally(params: SaveOrderParams): Promise<string> {
  ...
  await db.withTransactionAsync(async () => {
    ...
    for (const item of orderItems) {
      ...
      // INSERT order_item
      await db.runAsync(
        `INSERT INTO order_items (id, order_id, product_id, product_size_id, product_name_snapshot, size_label_snapshot, quantity, unit_price_snapshot, created_at, total_price)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          itemId,
          orderId,
