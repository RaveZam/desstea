import { getBranchTopProducts } from "../data/mock-data";
import { rankStyles } from "../../dashboard/data/mock-data";

interface BranchTopProductsProps {
  branchId: string;
}

export default function BranchTopProducts({ branchId }: BranchTopProductsProps) {
  const products = getBranchTopProducts(branchId);
  const maxRevenue = products[0]?.revenue ?? 1;

  return (
    <div className="bg-white rounded-2xl shadow-sm p-4">
      <h3 className="font-semibold text-gray-900">Top Selling Products</h3>
      <p className="text-xs text-gray-400 mt-0.5 mb-4">Ranked by revenue at this branch</p>
