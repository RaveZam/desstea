    ...
    <Modal open={open} onClose={onClose} title={combo.name} size="lg">
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between bg-[#FFF8F4] border border-[#F5C5A3] rounded-xl px-4 py-3.5">
          <div>
            <p className="text-[10px] font-semibold text-[#9B7B5F] uppercase tracking-widest mb-0.5">
              Combo Price
            </p>
            <p className="text-3xl font-bold text-[#6B4F3A]">
                      ...
                      <>
                        ...
                        {slot.selection_group && (
                          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-sky-50 text-sky-600 border border-sky-200">
                            Group:{" "}
                            {slot.selection_group.charAt(0).toUpperCase() +
                              slot.selection_group.slice(1)}
                          </span>
                        )}
                      </>
                    ) : (
                  ...
                  {slot.products.map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        {p.upgrade_price > 0 && (
                          <span className="shrink-0 text-xs font-semibold text-[#E8692A]">
                            +₱{p.upgrade_price.toFixed(2)}
                          </span>
                        )}
                      </div>
        ...
        {combo.slots.some((s) => s.products.length > 0) && (
          <div className="flex items-center justify-between border-t border-gray-100 pt-4">
            ...
            <span className="text-xl font-bold text-[#E8692A]">
              ₱{Number(combo.price).toFixed(2)}
            </span>
          </div>
        )}
